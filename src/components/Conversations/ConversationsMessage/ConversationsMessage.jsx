...
export default class extends React.Component {
  ...
  formatTimeToShow(time, now) {
    ...
    let timeToShow = "";
    if(timeSince > 60*24){
      timeToShow = ((time.getDate() < 10) ? '0'+time.getDate().toString():time.getDate().toString()) + '.' +
                    ((time.getMonth() < 9) ? '0'+(time.getMonth() + 1).toString():(time.getMonth() + 1).toString()) + '.' +
                    (time.getFullYear().toString().substring(2));
    } else {
      timeToShow = ((time.getHours() < 10) ? '0' + time.getHours().toString() : time.getHours().toString()) + ':'+
  ...
  formatTimeSince(time, now){
    let timeSinceInt = parseInt((now.getTime() - time.getTime())/(1000*60)) ;
    let timeSinceStr = "";
    if (timeSinceInt > 60*24){
      timeSinceInt = parseInt(timeSinceInt / (60*24));
      timeSinceStr = timeSinceInt.toString() + ((timeSinceInt === 1) ? ' dag':' dager');
    } else if (timeSinceInt > 60){
      timeSinceInt = parseInt(timeSinceInt / (60));
      timeSinceStr = timeSinceInt.toString() + ((timeSinceInt === 1) ? ' time':' timer');
    } else {
